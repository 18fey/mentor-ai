// app/auth/email-sent/EmailSentInner.tsx
"use client";
...
export function EmailSentInner() {
  ...
  const handleResend = async () => {
    if (!email) {
      setMessage(
        "メールアドレス情報が見つかりません。もう一度新規登録をお試しください。"
      );
      return;
    }
    setLoading(true);
    ...
    if (error) {
      setMessage(
        error.message ||
          "メールの再送信に失敗しました。時間をおいて再度お試しください。"
      );
    } else {
      setMessage(
  ...
  return (
    <div className="w-full max-w-md rounded-3xl border border-white/40 bg-white/70 p-8 text-center shadow-[0_18px_45px_rgba(15,23,42,0.12)] backdrop-blur-[30px]">
      ...
      {message && <p className="mb-3 text-xs text-sky-700">{message}</p>}
